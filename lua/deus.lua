...
function highlite:highlight_terminal(terminal_colors)
	for index, color in ipairs(terminal_colors) do
		vim.g["terminal_color_" .. (index - 1)] = vim.go.termguicolors and color[_PALETTE_HEX] or color[_PALETTE_CTERM]
	end
end

return setmetatable(highlite, {
	__call = function(self, normal, highlights, terminal_colors)
		...
		if not (vim.go.termguicolors or _USE_256) then
			vim.go.t_Co = "16"
		end
